...
namespace model::ltl {
    ...
    private:
        ...
        static const Formula &alloc(const Formula *formula) {
            _formulae.push_back(std::shared_ptr<const Formula>(formula));
            return *_formulae[_formulae.size() - 1];
        }

        static std::vector<std::shared_ptr<const Formula>> _formulae;
